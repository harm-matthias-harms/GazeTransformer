...
class AngularLoss2(nn.Module):
    ...
    def forward(self, input, target):
        loss = F.pairwise_distance(
            input, target, self.norm, self.eps, self.keepdim)
...
def CalAngularDist(x, y, eps=1e-6):
    ...
    eye2x = ScreenDist*ScreenDist + \
        torch.pow(x1 - ScreenCenterX, 2) + torch.pow(x2 - ScreenCenterY, 2)
    # the square of the distance between eye and y
    eye2y = ScreenDist*ScreenDist + \
        torch.pow(y1 - ScreenCenterX, 2) + torch.pow(y2 - ScreenCenterY, 2)
...
def acos_safe(x, eps=1e-6):
    ...
    buf[bad] = torch.acos(sign * (1 - eps)) - slope * \
        sign*(abs(x[bad]) - 1 + eps)
    return buf
