...
class FixationNet(nn.Module):
    def __init__(self, gazeSeqSize, headSeqSize, objectSeqSize, saliencySize, clusterPath):
        ...
        self.saliencyNum = int(
            self.saliencySize/(self.saliencyWidth*self.saliencyWidth))

        # load the cluster centers
        self.cluster = torch.from_numpy(
            np.load(clusterPath)).float().to(device)
        ...
        gazeSeqCNN1D_featureSize1 = floor(
            (self.gazeSeqLength - gazeSeqCNN1D_kernelSize1 + 1)/gazeSeqCNN1D_poolingRate1)
        self.gazeSeqCNN1D_outputSize = gazeSeqCNN1D_featureSize1 * gazeSeqCNN1D_outChannels1
        self.gazeSeqCNN1D = nn.Sequential(
            nn.Conv1d(in_channels=self.gazeFeatureNum,
                      out_channels=gazeSeqCNN1D_outChannels1, kernel_size=gazeSeqCNN1D_kernelSize1),
        ...
        headSeqCNN1D_featureSize1 = floor(
            (self.headSeqLength - headSeqCNN1D_kernelSize1 + 1)/headSeqCNN1D_poolingRate1)
        self.headSeqCNN1D_outputSize = headSeqCNN1D_featureSize1 * headSeqCNN1D_outChannels1
        self.headSeqCNN1D = nn.Sequential(
            nn.Conv1d(in_channels=self.headFeatureNum,
                      out_channels=headSeqCNN1D_outChannels1, kernel_size=headSeqCNN1D_kernelSize1),
        ...
        objectSeqCNN1D_featureSize1 = floor(
            (self.objectSeqLength - objectSeqCNN1D_kernelSize1 + 1)/objectSeqCNN1D_poolingRate1)
        objectSeqCNN1D_outChannels2 = 32
        objectSeqCNN1D_poolingRate2 = 2
        objectSeqCNN1D_kernelSize2 = 1
        objectSeqCNN1D_featureSize2 = floor(
            (objectSeqCNN1D_featureSize1 - objectSeqCNN1D_kernelSize2 + 1)/objectSeqCNN1D_poolingRate2)
        self.objectSeqCNN1D_outputSize = objectSeqCNN1D_featureSize2 * \
            objectSeqCNN1D_outChannels2
        self.objectSeqCNN1D = nn.Sequential(
            nn.Conv1d(in_channels=self.objectFeatureNum,
                      out_channels=objectSeqCNN1D_outChannels1, kernel_size=objectSeqCNN1D_kernelSize1),
            ...
            nn.Conv1d(in_channels=objectSeqCNN1D_outChannels1,
                      out_channels=objectSeqCNN1D_outChannels2, kernel_size=objectSeqCNN1D_kernelSize2),
        ...
        salCNN_imageSize1 = floor(
            (self.saliencyWidth - salCNN_kernelSize1 + 2*salCNN_padding1 + 1)/salCNN_poolingRate1)
        self.salCNN_outputSize = salCNN_imageSize1 * \
            salCNN_imageSize1 * salCNN_outChannels1
        self.SalCNN = nn.Sequential(
            nn.Conv2d(self.saliencyNum, salCNN_outChannels1,
                      kernel_size=salCNN_kernelSize1, padding=salCNN_padding1),
        ...
        # PrdFC Module
        prdFC_inputSize = self.gazeSeqCNN1D_outputSize + self.headSeqCNN1D_outputSize + \
            self.objectSeqCNN1D_outputSize + self.salCNN_outputSize
    ...
    def forward1(self, x):
        ...
        objectSeq = objectSeq.reshape(-1,
                                      self.objectSeqLength, self.objectFeatureNum)
        ...
        saliencyMap = saliencyMap.reshape(saliencyMap.size(
            0), self.saliencyNum, self.saliencyWidth, self.saliencyWidth)
